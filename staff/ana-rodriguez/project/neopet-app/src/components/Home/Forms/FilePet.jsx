...
function FilePet({ petId }) {
    ...
    const saveFile = (event) => {
        ...
        modifyPets(petId, pet, '').then((response) => {
            console.log(response);
        })
    }

    return <>
        <form className="file_container" onSubmit={event => saveFile(event)}>
            <h1 className="title_file"></h1>
            <div className="file">
                <label>Peso<input className="file_input" type="text" name="weigth" /></label><input className="file_input" type="text" name="weigth" />
                <label className="subtitle_file">Vacunas
                    <textarea name="noteVaccines" id="" cols="30" row="10"></textarea>
                </label>
                <label className="subtitle_file">Desparasitaciones
                    <textarea name="noteDeworming" id="" cols="30" rows="10"></textarea>
                </label>
                <label className="subtitle_file">Notas
