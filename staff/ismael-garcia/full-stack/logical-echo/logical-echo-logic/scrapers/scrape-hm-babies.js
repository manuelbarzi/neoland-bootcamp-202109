...
function scrapeHMBabies() {
    return (async () => {
        try {
            await scrapeCollection( {
                url: 'https://www2.hm.com/es_es/bebe/sostenibilidad/our-products.html',
                selector: '.item-link',
            ...
            logger.debug('end scraping H&M Babies Collection')
        } catch (error) {
            logger.error(error)
        }
    })()
