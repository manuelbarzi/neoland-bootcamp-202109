...
function scrapeMangoMan() {
    return (async () => {
        try {
            await scrapeCollection( {
                url: 'https://shop.mango.com/es/search?kw=committed&brand=he&origin=ultimas-busquedas',
                selector: '.bb1c_',
            ...
            logger.debug('end scraping Mango Man Collection')
        } catch (error) {
            logger.error(error)
        }
    })()
